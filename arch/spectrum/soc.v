...
/* Initialize attribute area to white-on-black so display is visible
 * before the ROM sets its own attributes */
integer init_i;
initial begin
    for (init_i = 0; init_i < 49152; init_i = init_i + 1)
        ram[init_i] = 8'h00;
    for (init_i = 16'h1800; init_i < 16'h1B00; init_i = init_i + 1)
        ram[init_i] = 8'h07;  // ink=white, paper=black
end
...
wire [8:0] dx = lcd_col - BORDER_H;   // 0-255 display x
wire [7:0] dy = lcd_row - BORDER_V;   // 0-191 display y

/* Spectrum bitmap address: 010 Y7Y6 Y2Y1Y0 Y5Y4Y3 X4X3X2X1X0 */
wire [12:0] bitmap_addr = {dy[7:6], dy[2:0], dy[5:3], dx[7:3]};

/* Attribute address: 110 Y7Y6Y5Y4Y3 X4X3X2X1X0 */
wire [12:0] attr_addr = {3'b110, dy[7:3], dx[7:3]};

/* Video RAM read — continuous, every pixel */
reg [7:0] vid_bitmap;
reg [7:0] vid_attr;

always @(posedge clk) begin
    if (lcd_de) begin
        vid_bitmap <= ram[bitmap_addr];
        vid_attr   <= ram[attr_addr];
    end
end
...
wire [3:0] pal_idx = in_display ? (use_ink ? ink_idx : paper_idx) : {1'b0, border_color};
