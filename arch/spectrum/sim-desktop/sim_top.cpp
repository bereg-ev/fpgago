/*
 * sim_top.cpp — ZX Spectrum Verilator + SDL2 desktop simulation
 *
 * Build:  cd arch/spectrum/sim-desktop && make
 * Run:    ./obj_dir/Vsoc
 *
 * Keys:
 *   ESC         — quit
...
int main(int argc, char** argv)
{
    ...
    while (running && !ctx->gotFinish())
    {
        ...
        if (cycle_count % 5000000 == 0 && cycle_count > 5000000 && cycle_count <= 80000000) {
            fprintf(stderr, "[%lldM] TV_FLAG=$%02X FLAGS=$%02X\n",
                    cycle_count/1000000,
                    (unsigned)top->dbg_tvflag,
                    (unsigned)top->dbg_flags);
        }
