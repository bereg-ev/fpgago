...
module DELAYG #(
  parameter DEL_MODE  = "USER_DEFINED",   /* accepted and ignored in sim */
...
module dual_port_ram_4k_18 #(parameter HI_HALF = 0) (
    ...
    /* Port B is writable (QSPI app loader): each instance owns one 16-bit
     * half of the shared 32-bit word layout, selected by HI_HALF. */
    always @(posedge clk_b)
        if (we_b) begin
            if (HI_HALF) mem[addr_b][31:16] <= din_b[15:0];
            else         mem[addr_b][15:0]  <= din_b[15:0];
        end else
