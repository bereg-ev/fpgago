 ...
 /* 2K words deep.  With only 9 of the 18 bits used (the byte-lane dataram)
  * yosys trims the width and 2048x9 = 18432 bits maps to EXACTLY ONE DP16KD —
  * half the EBR of ram_4k_18 for the same shape.  See soc.v SMALL_DATARAM. */
 module ram_2k_18 (
    input wire clk_a,
    input wire we_a,
...
// Correct instantiation of the DP16KD dual-port RAM
    ...
    DP16KD #(
        ...
        /* x18 mode: AD[1:0] are ACTIVE-HIGH BYTE-WRITE ENABLES, not address
         * bits (see yosys lattice/brams_map_16kd.v).  Tied low they MASK ALL
         * WRITES (reads don't care) — cost the QSPI app loader a board
         * round: every APP_WRITE was silently dropped.  Tie high. */
        .ADA0(1'b1),                          // byte-write enable low half
        .ADA1(1'b1),                          // byte-write enable high half
        .ADA2(1'b0),
        .ADA3(1'b0),
        ...
        .ADB0(1'b1),                          // byte-write enable low half
        .ADB1(1'b1),                          // byte-write enable high half
        .ADB2(1'b0),
        .ADB3(1'b0),
