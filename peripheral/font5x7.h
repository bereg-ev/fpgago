/*
 * font5x7.h — 5×7 bitmap font for 96 printable ASCII characters (0x20..0x7F)
 *
 * Shared across all games and arch HALs.  Games include via
 * `#include "font5x7.h"` (the build system adds -Iperipheral).
 *
 * Each entry is 7 bytes, one per row (top to bottom).
 * Within a byte, bits [4:0] represent columns 0..4 (left to right):
 ...
 *   for (row = 0; row < 7; row++)
 *       for (col = 0; col < 5; col++)
 *           if (glyph[row] & (0x10 >> col))  draw_pixel(x+col, y+row);
 */

#ifndef FONT5X7_H
#define FONT5X7_H

/* Use unsigned int (32-bit) to avoid byte-load instructions on RISC2 */
static const unsigned int font5x7[96][7] = {
    ...
    /* 0x2E '.' */ { 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x00 },
    ...
    /* 0x41 'A' */ { 0x0E, 0x11, 0x11, 0x1F, 0x11, 0x11, 0x00 },
    ...
    /* 0x6D 'm' */ { 0x00, 0x00, 0x1A, 0x15, 0x15, 0x15, 0x11 },
    /* 0x6E 'n' */ { 0x00, 0x00, 0x1E, 0x11, 0x11, 0x11, 0x11 },
    /* 0x6F 'o' */ { 0x00, 0x00, 0x0E, 0x11, 0x11, 0x11, 0x0E },
    /* 0x70 'p' */ { 0x00, 0x00, 0x1E, 0x11, 0x11, 0x1E, 0x10 },
    /* 0x71 'q' */ { 0x00, 0x00, 0x0F, 0x11, 0x11, 0x0F, 0x01 },
