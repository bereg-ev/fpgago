...
initial
  begin
    /* tx MUST init to 1: its async reset value is 1 (UART idle-high), and an
     * ECP5 FF cannot power up to a value different from its async set/reset
     * value (yosys DFFLEGALIZE rejects the combination). */
    {otxen, rxen, rxdata, txstate, rxstate, out, txsh, rxsh, txcnt} <= 0;
    tx <= 1;
  end
