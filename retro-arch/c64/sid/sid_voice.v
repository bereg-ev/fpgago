/*
 * sid_voice.v — SID voice: oscillator, waveform gen, envelope, DCA
 *
 * yosys-safe port of the MiSTer C64 sid_voice.sv (GPL).  Behavior
 * unchanged (including the upstream mode?TTL_6581:TTL_8580 selection
 * in the DCA keep counter, quirky-looking but kept verbatim);
 * always-block-local registers hoisted to module scope, size casts and
 * '1 fills expanded to plain Verilog.
 */
...
localparam signed [21:0] VOICE_DC_6581    = 22'sd212160;    // 'h340*'hff, measured from samples.
localparam signed [21:0] VOICE_DC_8580    = 22'sd0;         // No DC offsets in the MOS8580.
...
reg [11:0] saw_tri;
reg        pulse;
reg        ng_clk, ng_clk_d;   // noise LFSR clocking (upstream: clk/clk_d)
reg        osc_edge;
reg [23:0] noise_age;
reg [22:0] lfsr_noise;
always @(posedge clock) begin
	if (reset) begin
		...
		if (ce_1m) begin
			...
			ng_clk     <= ~(reset || test_ctrl || (~osc_edge & oscillator[19]));
			ng_clk_d   <= ng_clk;
			...
			if (~ng_clk) begin
				if (noise_age >= (mode ? NOISE_TTL_8580 : NOISE_TTL_6581)) noise <= 12'hFFF;
				else noise_age <= noise_age + 1'd1;
			end
			else begin
				noise_age <= 0;
				if (ng_clk & ~ng_clk_d) begin
...
always @(posedge clock) begin
	if (ce_1m) begin
		if (control[7:4]) begin
			...
			dac_out  <= {1'b0, norm_dac | {comb, 4'b0000}} - (mode ? WAVEFORM_DC_8580 : WAVEFORM_DC_6581);
		end
		else if (keep_cnt) keep_cnt <= keep_cnt - 1'd1;
		else dac_out <= 0;

		env_dac <= mode ? {1'b0, envelope} : {1'b0, env_6581};
