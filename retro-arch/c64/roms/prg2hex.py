...
def convert(infile, hexfile):
    ...
    # boot_typer script ROM (typed by RTL after boot — replaces the old
    # C++-side autorun in sim_top.cpp, same path as future hardware).
    AUTORUN_ROM = 128
    if len(autorun) + 1 > AUTORUN_ROM:
        print(f'Error: autorun script is {len(autorun)} bytes (> {AUTORUN_ROM - 1})')
        sys.exit(1)
    script = autorun.encode() + b'\x00' * (AUTORUN_ROM - len(autorun))
    autorun_hex = os.path.join(os.path.dirname(os.path.abspath(hexfile)),
                               'autorun.hex')
    with open(autorun_hex, 'w') as f:
        f.write('\n'.join(f'{b:02x}' for b in script) + '\n')

    print(f'  -> {hexfile}, {autorun_hex}')
