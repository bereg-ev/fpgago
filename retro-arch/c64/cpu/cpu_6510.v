...
module cpu_6510(
    ...
    // Output to SoC.  Input-mode bits (DDR=0) are NOT driven low: P0-P2
    // (LORAM/HIRAM/CHAREN) and P4 (cassette sense) have external pull-ups on
    // the C64 board, so they float HIGH when switched to input.  Games that
    // clear zero page (writing $00 to DDR+port) rely on this — the memory
    // map must stay BASIC+KERNAL+I/O, not bank everything out.
    localparam [5:0] PORT_PULLUP = 6'b010111;
    assign port_out = (port_reg[5:0] & ddr[5:0]) | (PORT_PULLUP & ~ddr[5:0]);
