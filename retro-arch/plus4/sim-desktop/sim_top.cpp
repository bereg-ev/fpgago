...
int main(int argc, char** argv)
{
     ...
     * (LOAD vector $032E, SETNAM→$AB/$AF, SETLFS→$AD, reloc addr $B4). 64KB. */
...
#define SIM_TITLE "Commodore Plus/4 — FPGAgo Simulation"
    ...
    while (running && !ctx->gotFinish())
    {
        ...
        /* Plus/4 8501 CPU address trace (sim tracing of the KERNAL). */
        if (getenv("PLUS4_PC")) {
        ...
        /* QSPI fastload debug: RAM dump at exit (QSPI_RAMDUMP=hexaddr) */
        static bool qspi_dumped = false;
        if (!qspi_dumped && getenv("QSPI_RAMDUMP") && g_sim_us > 5500000) {
            qspi_dumped = true;
            unsigned base = strtoul(getenv("QSPI_RAMDUMP"), NULL, 16);
            for (int r = 0; r < 4; r++) {
                fprintf(stderr, "[ram] %04X:", base + r*16);
                for (int i = 0; i < 16; i++)
                    fprintf(stderr, " %02x",
                            top->rootp->soc__DOT__ram[(base + r*16 + i) & 0xFFFF]);
                fprintf(stderr, "\n");
            }
        }

        /* QSPI fastload debug: log first CPU touches of the hook/window */
        if (getenv("QSPI_DEBUG")) {
            static uint16_t qprev = 0;
            uint16_t a = (uint16_t)top->dbg_cpu_addr;
            if (a != qprev &&
                (a == 0xF04A || (a >= 0xFD60 && a <= 0xFDCF) ||
                 (a >= 0xFE20 && a <= 0xFE22))) {
                static int budget = 60;
                if (budget > 0) { budget--;
                    fprintf(stderr, "[qspi-cpu] %uus addr=$%04X\n", g_sim_us, a);
                }
            }
            qprev = a;
        }
